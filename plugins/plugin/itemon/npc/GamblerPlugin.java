...
public class GamblerPlugin extends PluginContext {
    ...
    protected boolean itemOnNpc(Player player, ItemOnNpcEvent event) {
        ...
        if (item.isStackable())
        {
            factory.sendNpcChat(1012, Expression.HAPPY, "Oooh some " + item.getName() + "...", "How many of those would you like to bet?");
            factory.onAction(() -> World.schedule(1, () -> player.send(new SendInputAmount("How many " + item.getName() + " would you like to bet?", 10, input -> bet(factory, Integer.parseInt(input))))));
            factory.execute();
        }
        else
        {
