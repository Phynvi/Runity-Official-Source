...
public class AmuletOfEternalGloryPlugin extends PluginContext {
    ...
    /*    if (equipment) {
            ...
            player.message("<col=7F007F>" + (index == 5 ? "Tip: Amulet of eternal glory has unlimited charges!"
            		: "Your amulet of eternal glory has " + Utility.convertWord(index).toLowerCase() + "charge" + (index == 1 ? "" : "s") + " remaining."));
        }*/
    }

  /*  private int getIndex(int item) {
        int index = + 1;
        for (int amulet = 1; amulet < AMULETS.length; amulet++) {
