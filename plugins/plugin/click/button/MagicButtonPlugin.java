        ...
        switch (button) {
            ...
            case 30000:
                if (PlayerRight.isManagement(player)) {
                    player.dialogueFactory.sendOption("Home", () -> Teleportation.teleport(player, Config.DEFAULT_POSITION), "Donator Zone", () -> Teleportation.teleport(player, Config.DONATOR_ZONE), "Staff Zone", () -> Teleportation.teleport(player, Config.STAFF_ZONE)).execute();
                    return true;
                }
                if (PlayerRight.isDonator(player)) {
                    player.dialogueFactory.sendOption("Home", () -> Teleportation.teleport(player, Config.DEFAULT_POSITION), "Donator Zone", () -> Teleportation.teleport(player, Config.DONATOR_ZONE)).execute();
                    return true;
                }
