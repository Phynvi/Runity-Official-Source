...
public class SkillSetButtonPlugin extends PluginContext {
    ...
    protected boolean onClick(Player player, int button) {
        ...
        player.dialogueFactory.sendOption("Set desired level", () -> player.dialogueFactory.onAction(() -> SkillSet.set(player, data)), "Set regular level", () -> player.dialogueFactory.onAction(() -> {
            ...
            player.dialogueFactory.sendStatement("You have successfully set your <col=255>" + Skill.getName(data.skill) + "</col> level", "back to <col=255>" + player.skills.getMaxLevel(data.skill) + "</col>.", "You will gain experience now (unless manually toggled off).").execute();
        }), "How does this work?", () -> {}).execute();
       // }
        return true;
