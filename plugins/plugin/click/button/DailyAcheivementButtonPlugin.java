...
public class DailyAcheivementButtonPlugin extends PluginContext {
	...
	//basically copied the current achievement system, and suited it to a daily achievement system.
    ...
    protected boolean onClick(Player player, int button) {
        if (DAILYACHIEVEMENT_BUTTONS.containsKey(button)) {
            ...
            player.send(new SendMessage("You have completed " + (progress > 100 ? 100 : progress) + "% of this achievement" + (completed ? "." : remaining), MessageColor.DARK_BLUE));
            return true;
        }
        ...
        if (DAILYACHIEVEMENT_TITLES.containsKey(button)) {
            ...
            player.send(new SendMessage("You have completed a total of " + completed + "/" + total + " " + difficulty.name().toLowerCase() + " achievements.", MessageColor.DARK_BLUE));
            return true;
        }
        return false;
    }
