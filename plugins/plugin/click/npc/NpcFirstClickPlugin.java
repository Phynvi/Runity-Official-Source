...
import io.battlerune.content.dialogue.impl.*;
           ...
           /* Ironman shop */
            case 311: {
                ...
                if (PlayerRight.isIronman(player) || PlayerRight.isDeveloper(player)) {
                    factory.sendNpcChat(id, "Welcome my friend.", "How may I help you?").sendOption("Claim armour", player.playerAssistant::claimIronmanArmour, "View shop", () -> factory.onAction(() -> Store.STORES.get("Ironman Store").open(player)), "Nevermind", factory::clear).execute();
                } else {
                    factory.sendNpcChat(id, "I have no buisness with you plebs, taking the easy route", "You're not an Ironman, you wimp.");
            ...
            case 5919:
                if (PlayerRight.isDeveloper(player)) {
                    ...
                    player.dialogueFactory.sendStatement("These store requires a client update", "Please wait till the official update release!").execute();
                }
                break;
