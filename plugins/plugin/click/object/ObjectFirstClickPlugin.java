...
public class ObjectFirstClickPlugin extends PluginContext {
    ...
    */
        ...
        /*    case 29789:
            //	player.animate(ANIM);
                if (player.getX() == 3311 && player.getY() == 5368) {
                    player.walk(new Position(3310, player.getX()), true);
                    player.walk(new Position(5373, player.getY()), true);
...
*/
            ...
            case 23676:
                ...
                break;
            /**
             * Basically, copy the code. change the case ID So it matches the Object ID.
             * so if i wanted to do lava dragon portals which is 23676.
             * yeah so this is the only file you gotta edit for the portals.
             */


            case 29042:
                if (player.barrowsKillCount == 6) {
                    player.send(new SendMessage("@or2@You have completed @or1@ Barrows Minigame @red@" + player.getName() + "!"));
            ...
            case 24064:
                ...
                player.message("Welcome to Near Reality's Gambling zone!");
                break;
                
                    	
            case 3399:
                if (player.inventory.getFreeSlots() <= 2) {
                    player.dialogueFactory.sendNpcChat(1152, "You need atleast 2 free slots to take the free supplies.").execute();

                    return true;
                }
                if (!player.takeobj.elapsed(2, TimeUnit.MINUTES)) {
                    player.dialogueFactory.sendNpcChat(1152, "You can only do this once every " + 2 + " minutes!", "Time Passed: " + Utility.getTime(player.takeobj.elapsedTime())).execute();
            ...
            case 29771:
                if (player.inventory.getFreeSlots() <= 13) {
                    player.dialogueFactory.sendNpcChat(1152, "You need atleast 13 free slots to take the free supplies.").execute();
            ...
            case 26281:
            	   if (player.inventory.getFreeSlots() <= 2) {
                       player.dialogueFactory.sendNpcChat(345, "You need atleast 2 free slots to take the free supplies.").execute();

                       return true;
                   }
                   if (!player.takeAntiFireshieldDelay.elapsed(2, TimeUnit.MINUTES)) {
                       player.dialogueFactory.sendNpcChat(345, "You can only do this once every " + 2 + " minutes!", "Time Passed: " + Utility.getTime(player.takeAntiFireshieldDelay.elapsedTime())).execute();
            ...
            case 32293:
         	   if (player.inventory.getFreeSlots() <= 2) {
                    player.dialogueFactory.sendNpcChat(7481, "You need atleast 2 free slots to take the free supplies.").execute();

                    return true;
                }
                if (!player.takeHammersDelay.elapsed(2, TimeUnit.MINUTES)) {
                    player.dialogueFactory.sendNpcChat(7481, "You can only do this once every " + 2 + " minutes!", "Time Passed: " + Utility.getTime(player.takeHammersDelay.elapsedTime())).execute();

                    return true;
                }
                player.dialogueFactory.sendNpcChat(7481, "You've stolen the hammers before anyone could see you!").execute();
            ...
            case 15931:
                 ...
                 if (!player.takeLogsDelay.elapsed(2, TimeUnit.MINUTES)) {
                     player.dialogueFactory.sendNpcChat(7481, "You can only do this once every " + 2 + " minutes!", "Time Passed: " + Utility.getTime(player.takeLogsDelay.elapsedTime())).execute();
            ...
            case 2873:
                player.dialogueFactory.sendStatement("Would you like to purchase a god cape for 25,000 coins?").sendOption("Yes", () -> {
                    if (!player.quest.completed(QuestManager.KOLODIONS_ARENA)) {
                        player.send(new SendMessage("You must complete the quest 'Kolodion's Arena' before doing this!"));
                        return;
...
//                player.message("Personal stores are currently disabled!");
            ...
            /* LMS Coffer */
            case 29087:
                player.dialogueFactory.sendStatement("You have " + Util.toNiceString(player.getLmsCoffer()) + " coins in your coffer").sendOption("Deposit", () -> {
                    player.dialogueFactory.onAction(() -> World.schedule(1, () -> player.send(new SendInputAmount("How much would you like to deposit?", 10, input -> {
                        int amount = Integer.parseInt(input);
                        if (player.inventory.contains(new Item(995, amount)))
                        {
                            player.inventory.remove(new Item(995, amount));
                            player.lmsCoffer += amount;
                            player.send(new SendMessage("You have successfully entered " + Util.toNiceString(amount) + " coins into the coffer."));
                        }
                        else
                        {
                    ...
                    player.dialogueFactory.onAction(() -> World.schedule(1, () -> player.send(new SendInputAmount("How much would you like to withdraw?", 10, input -> {
                        int amount = Integer.parseInt(input);
                        if (player.lmsCoffer >= amount)
                        {
                            if (player.inventory.hasCapacityFor(new Item(995, amount)))
                            {
                                ...
                                player.lmsCoffer -= amount;
                                player.send(new SendMessage("You have successfully withdrawn " + Util.toNiceString(amount) + " coins from the coffer."));
                            }
                            else
                            {
                        ...
                        {
                            player.send(new SendMessage("Sorry, you only have " + Util.toNiceString(player.lmsCoffer) + " coins in your coffer."));
                        }
                    })))).execute();
                }).execute();
                break;
