    ...
    public Path find(Position origin, int originWidth, int originLength, Position target, int targetWidth, int targetLength) {
        ...
        while (tail != tileQueueX.size() && tileQueueX.size() < pathLength) {
            ...
            if (curY > 0 && via[curX][curY - 1] == 0
                    && traversable(position, originWidth, SOUTH)) {
                tileQueueX.add(curX);
            ...
            if (curY < 104 - 1 && via[curX][curY + 1] == 0
                    && traversable(position, originWidth, NORTH)) {
                tileQueueX.add(curX);
                tileQueueY.add(curY + 1);
                via[curX][curY + 1] = 4;
                cost[curX][curY + 1] = thisCost;
            ...
            if (curX > 0 && curY > 0 && via[curX - 1][curY - 1] == 0
                    && traversable(position, originWidth, SOUTH_WEST)
                    && traversable(position, originWidth, SOUTH)
                    && traversable(position, originWidth, WEST)) {
                tileQueueX.add(curX - 1);
                tileQueueY.add(curY - 1);
            ...
            if (curX > 0 && curY < 104 - 1 && via[curX - 1][curY + 1] == 0
                    && traversable(position, originWidth, NORTH_WEST)
                    && traversable(position, originWidth, NORTH)
                    && traversable(position, originWidth, WEST)) {
                tileQueueX.add(curX - 1);
                tileQueueY.add(curY + 1);
            ...
            if (curX < 104 - 1 && curY > 0 && via[curX + 1][curY - 1] == 0
                    && traversable(position, originWidth, SOUTH_EAST)
                    && traversable(position, originWidth, SOUTH)
                    && traversable(position, originWidth, EAST)) {
                tileQueueX.add(curX + 1);
                tileQueueY.add(curY - 1);
            ...
            if (curX < 104 - 1 && curY < 104 - 1 && via[curX + 1][curY + 1] == 0
                    && traversable(position, originWidth, NORTH_EAST)
                    && traversable(position, originWidth, NORTH)
                    && traversable(position, originWidth, EAST)) {
                tileQueueX.add(curX + 1);
                tileQueueY.add(curY + 1);
