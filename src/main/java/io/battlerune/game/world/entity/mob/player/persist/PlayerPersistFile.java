...
public final class PlayerPersistFile implements PlayerPersistable {
    ...
    private static final PlayerJSONProperty[] PROPERTIES = {
            ...
            new PlayerJSONProperty("achievement") {
                @Override
                void read(Player player, JsonElement property) {
                    player.playerAchievements = GSON.fromJson(property, new TypeToken<HashMap<AchievementKey, Integer>>() {}.getType());
                }
                ...
                @Override
                void read(Player player, JsonElement property) {
                    player.playerAchievements1 = GSON.fromJson(property, new TypeToken<HashMap<DailyAchievementKey, Integer>>() {}.getType());
                }

                @Override
                Object write(Player player) {
                    return GSON.toJsonTree(player.playerAchievements1, new TypeToken<HashMap<DailyAchievementKey, Integer>>() {}.getType());
                }
            },
