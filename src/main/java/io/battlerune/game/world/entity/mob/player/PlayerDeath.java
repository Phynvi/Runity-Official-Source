	...
	public void death() {
		...
		case PLAYER:
			...
			if (mob.isBot) {
				...
				playerKiller.message("<col=295EFF>You were rewarded with " + playerKiller.getpkPoints()+ " points for that kill. You now have: " + Utility.formatDigits(playerKiller.pkPoints) + ".");
				return;
			}
	  ...
	  /*	if (killer.isPlayer()) {
		...
		if (!safe) {
			...
			if (mob.right == PlayerRight.HARDCORE_IRONMAN) {
				...
				World.sendMessage("<col=FF0000>" + Config.SERVER_NAME + " <col=" + mob.right.getColor() + ">"
