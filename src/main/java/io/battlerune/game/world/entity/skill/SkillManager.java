...
public class SkillManager {
    ...
    public void addExperience(int id, double experience, boolean levelUp, boolean counter) {
       ...
       /*   if(player.equipment.contains(20366) && Area.inWilderness(player) || Area.inWilderness(player) || Area.inWildernessResource(player) || Area.inWildernessCourse(player)) {
            modified_experience = experience * 3;
           player.message("You are now recieving 3x the normal EXP Rate due to your amulet!");
        }*/
...
//            ClanManager.addExperience(player, experience);
    ...
    private void updateSkill(Player player, int maxLevel, int newMax, Skill skill, boolean levelUp) {
        if (maxLevel < 99 && newMax != maxLevel) {
            ...
            if (levelUp) {
               //adam should disable levling up stopping skills anaomly player.action.clearNonWalkableActions(); // causes flax task to stop upon leveling up
                showLevelUpInterface(player, skill);
                if (newMax == 99)
                    World.sendMessage("<col=7B44B3>Near Reality: <col=" + player.right.getColor() + ">" + player.getName() + " </col>has just reached level 99 in <col=7B44B3>" + Skill.getName(skill.getSkill()) + "</col>!");
            } else {
