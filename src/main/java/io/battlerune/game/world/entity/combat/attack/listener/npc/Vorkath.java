...
public class Vorkath {//extends SimplifiedListener<Npc> { 
	...
	private static final Animation shoot = new Animation(7952);
	private static final Animation up = new Animation(7960);

	private static final int blue = 1479;
	private static final int blueBlast = 1480;
	private static final int pink = 1471;
	private static final int pinkBlast = 1473;
	private static final int green = 1470;
	private static final int greenBlast = 1472;
	private static final int red = 1481;
	private static final int redBlast = 157;
		...
		if (random == 0) {
			...
			TaskManager.submit(new Task(1, dragon, false) {
				...
				public void execute() {
					...
					switch (tick) {
					case 0:
						new Projectile(dragon, victim, fire, 44, 3, 31, 31, 0).sendProjectile();
						break;
					case 2:
			...
			TaskManager.submit(new Task(1, dragon, true) {
				...
				public void execute() {
					...
					switch (tick) {
					case 0:
						new Projectile(dragon, victim, ranged, 44, 3, 25, 31, 0).sendProjectile();
						break;
					case 2:
						dragon.setChargingAttack(false).getCombatBuilder().setAttackTimer(6);
						stop();
			...
			TaskManager.submit(new Task(1, dragon, true) {
				...
				public void execute() {
					...
					switch (tick) {
					case 0:
						new Projectile(dragon, victim, pink, 44, 3, 31, 31, 0).sendProjectile();
						break;
					case 2:
			...
			TaskManager.submit(new Task(1, dragon, true) {
				...
				public void execute() {
					...
					switch (tick) {
					case 0:
						new Projectile(dragon, victim, green, 44, 3, 31, 31, 0).sendProjectile();
						break;
					case 2:
			...
			Npc position = new Npc(1, victim.getPosition());
				...
				public void execute() {
					...
					case 3:
						...
						if (target.getPosition() == savew)
							target.dealDamage(
									new Hit(400 + RandomUtility.getRandom(300), Hitmask.RED, CombatIcon.BLOCK));
			...
			TaskManager.submit(new Task(1, dragon, true) {
				...
				public void execute() {
					...
					switch (tick) {
					case 0:
						new Projectile(dragon, victim, blue, 44, 3, 31, 31, 0).sendProjectile();
						break;
					case 2:
