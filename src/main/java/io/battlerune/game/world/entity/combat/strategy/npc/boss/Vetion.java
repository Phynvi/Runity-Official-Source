...
public class Vetion extends MultiStrategy {
	...
	public void finishIncoming(Mob attacker, Npc defender) {
		...
		Position[] innerBoundary = Utility.getInnerBoundaries(defender.getPosition().transform(-2, -2),
				defender.width() + 4, defender.length() + 4);
	...
	private static final class Magic extends NpcMagicStrategy {
		...
		public void start(Npc attacker, Mob defender, Hit[] hits) {
			Position[] bounds = Utility.getInnerBoundaries(defender.getPosition().transform(-1, -1),
					defender.width() + 2, defender.length() + 2);
			...
			for (int index = 0; index < 3; index++) {
				...
				combatProjectile.getProjectile()
						.ifPresent(projectile -> World.sendProjectile(attacker, bound, projectile));
				World.sendGraphic(new Graphic(775, 100), bound);
				World.schedule(CombatUtil.getHitDelay(attacker, defender, CombatType.MAGIC) + 2,
						() -> RegionManager.forNearbyPlayer(bound, 0, player -> {
							player.speak("OUCH!");
