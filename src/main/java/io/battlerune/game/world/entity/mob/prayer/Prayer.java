 ...
 *
 * @author Michael
 * @author Daniel this is the enum for the prayer drain rates and what not. we
 *         can only set the drainrate as an instance otherwise it will affect
 *         all the players instead of only one sers? let me test it
 */
public enum Prayer {
	 ...
	 * @param button    The button id.
	 */
	Prayer(String name, int level, int drainRate, int config, int qConfig, int button, CombatListener<Mob> listener,
			Type... types) {
		this.name = name;
		this.level = level;
		this.drainRate = drainRate;
	...
	private static final Collection<Prayer> OFFENSIVE = ImmutableList.of(CLARITY_OF_THOUGHT, IMPROVED_REFLEXES,
			INCREDIBLE_REFLEXES, SHARP_EYE, HAWK_EYE, EAGLE_EYE, MYSTIC_WILL, MYSTIC_LORE, MYSTIC_MIGHT, CHIVALRY,
			PIETY, RIGOUR, AUGURY);

	private static final Collection<Prayer> AGGRESSIVE = ImmutableList.of(BURST_OF_STRENGTH, SUPERHUMAN_STRENGTH,
			ULTIMATE_STRENGTH, SHARP_EYE, HAWK_EYE, EAGLE_EYE, MYSTIC_WILL, MYSTIC_LORE, MYSTIC_MIGHT, CHIVALRY, PIETY,
			RIGOUR);

	private static final Collection<Prayer> DEFENSIVE = ImmutableList.of(THICK_SKIN, ROCK_SKIN, STEEL_SKIN, CHIVALRY,
			PIETY, RIGOUR, AUGURY);

	public static final Collection<Prayer> OVERHEAD = ImmutableList.of(PROTECT_FROM_MAGIC, PROTECT_FROM_RANGE,
	...
	public boolean canToggle(Player player) {
		...
		if ((this == PROTECT_FROM_MAGIC || this == PROTECT_FROM_MELEE || this == PROTECT_FROM_RANGE)
				&& Area.inEventArena(player)) {
			return false;
		}
		return PlayerRight.isPriviledged(player) || (this != RIGOUR && this != AUGURY && this != PRESERVE)
				|| player.unlockedPrayers.contains(this);
	}
