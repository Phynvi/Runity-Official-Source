...
/**
 * <h2>A modified DataBus implementation specifically for player-related events.</h2>
 *
 * <p>
 ...
 *
 * {@link PlayerEventListener}'s must subscribe to the bus
 * in order to recieve {@link Event}'s send by the bus.
 * {@link PlayerEventListener}'s can then decide which {@link Event}'s
 * they are interested in.
 *
 * The difference between this implementation and a regular DataBus implementation
 * is that {@link Player} is specified when publishing events.
 * Also this implementation knows if an {@link Event} was successfully handled
 * by a {@link PlayerEventListener}. If the {@link Event} was handled
 * then the {@link PlayerEventListener} chain break and the event does not
 * propagate any further across the chain.
 * </p>
 *
...
public final class PlayerDataBus {
    ...
    /**
     * Subscribes a listener to this bus. If a listener
     * subscribes to this bus then that listener can receive events
     * sent by the bus.
     *
     * @param listener
     *      The listener that is subscribing to this bus.
     */
    ...
    /**
     * Unsubscribes a listener from this bus. If a listener
     * is unsubscribed that listener no longer receives messages
     * sent by the bus.
     *
     * @param listener
     *      The listener that is unsubscribing.
     */
    ...
    /**
     * Sends an {@link Event} to all subscribed event listeners.
     * If a listener is able to handle the message the chain breaks.
     * This is to prevent further propagating of the event.
     *
     * @param player
     *      The player that is sending the event.
     *
     * @param event
     *      The event that is going to be sent to all listeners.
     *
