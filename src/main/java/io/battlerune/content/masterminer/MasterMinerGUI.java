...
public class MasterMinerGUI {
    ...
    protected void updateMobDetails() {
        for (int i = 0; i < 8; i++)
        {
            ...
            //player.send(new SendString("Price: " + toNiceString(data.mobData[i].getPrice()), 24408 + i * INCREMENT));
            player.send(new SendTooltip("This does absolutely fucking nothing " + toNiceString(data.mobData[i].getGPS()), 24409 + i * INCREMENT));
            //player.send(new SendString(toNiceString(data.mobData[i].getGPS()), 24410 + i * INCREMENT));

            //player.send(new SendString(data.mobData[i].getName(), 24665 + i*4));
            //player.send(new SendString("- Gives " + toNiceString(data.mobData[i].getGPS()) + " gold per second.", 24666 + i*4));
    ...
    private void populateStatsBar() {
        ...
        for (int i = 0; i < 20; i++) statsArray[i] = "";
