...
public class Util {
    public static String toNiceString(long value) {
        ...
        if (value == Long.MIN_VALUE) return toNiceString(Long.MIN_VALUE + 1);
        if (value < 0) return "-" + toNiceString(-value);
        if (value < 100000) return Long.toString(value); //deal with easy case
        if (value < 10000000) return value/1000 + "k";
        if (value < 10000000000L) return value/1000000 + "m";
