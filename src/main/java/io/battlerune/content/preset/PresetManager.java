...
public class PresetManager {
	...
	public void refresh() {
		...
		for (int i = 0; i < SIZE; i++) {
			...
			String defaultName = locked ? "<col=F23030>Locked"
					: (preset[i] == null ? "Slot " + (i + 1) : preset[i].getName());
		...
		for (int index = 0; index < 7; index++) {
			player.send(new SendString(preset[slot] == null ? (index == 3 ? "10" : "1")
					: ((preset[slot].getSkill()[index] == 0 ? (index == 3 ? 10 : 1) : preset[slot].getSkill()[index])),
					57014 + index));
		}
		...
		player.send(new SendString(
				"<col=BD23E8>" + (preset[slot] == null ? "Not set" : preset[slot].getSpellbook().getName()), 57011));
	...
	public void upload() {
		...
		player.dialogueFactory.sendOption("Upload preset", () -> {
			...
			preset[slot] = new Preset(preset[slot].getName(), player.inventory.toArray(),
					player.equipment.getEquipment(), skill, prayerBook, player.spellbook);
	...
	public void activate() {
		...
		player.send(
				new SendMessage("You preset has been activated. If you are missing some items it is because you did"));
	...
	public void delete() {
		...
		player.dialogueFactory
				.sendOption("Delete " + preset[slot].getName() + " (<col=f44259>CANT BE UNDONE</col>)", () -> {
					preset[slot] = null;
					refresh();
	...
	public void openSettings() {
		...
		factory.sendOption("View current settings", () -> {
			factory.onAction(() -> {
				factory.sendInformationBox("<col=3E74B8>Preset Settings",
						"</col>Open on death: " + (deathOpen ? "<col=0E8716>Enabled" : "<col=F01818>Disabled"),
						"</col>Automatic deposit: " + (autoDeposit ? "<col=0E8716>Enabled" : "<col=F01818>Disabled"))
						.execute();
			});
		}, "Toggle open on death", () -> factory.onAction(() -> {
			deathOpen = !deathOpen;
			factory.sendStatement(
					"The preset interface will now " + (deathOpen ? "" : "<col=F01818>not</col> ") + "open on death",
					"automatically.").execute();
		}), "Toggle automatic deposit", () -> {
			factory.onAction(() -> {
				autoDeposit = !autoDeposit;
				factory.sendStatement("Your items will now " + (autoDeposit ? "" : "<col=F01818>not</col> ")
						+ "automatically deposit on activation.").execute();
			});
