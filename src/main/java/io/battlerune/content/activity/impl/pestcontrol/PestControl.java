...
public class PestControl extends GroupActivity {
    ...
    /** The portals. */
    protected final Portal[] portals = new Portal[]{
    ...
    protected boolean clickObject(Player player, ObjectInteractionEvent event) {
        if (event.getObject().getId() == 14314) {
            removeActivity(player);
          //  player.send(new SendMessage("You are trapped in my MINIGAME HAHAH! You can only leave once the minigame is completed."));
            return true;
        }
        return false;
    }
