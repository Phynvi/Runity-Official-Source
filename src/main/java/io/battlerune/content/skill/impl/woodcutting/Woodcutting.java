...
public final class Woodcutting extends Skill {
    ...
    protected boolean clickObject(Player player, ObjectInteractionEvent event) {
        ...
        if (event.getType() == ObjectInteractionEvent.InteractionType.SECOND_CLICK_OBJECT && tree.tree[0] == 2092) //AFK TREE
        {
        ...
        if (event.getType() == ObjectInteractionEvent.InteractionType.THIRD_CLICK_OBJECT && tree.tree[0] == 2092) //AFK TREE
        {
            Integer left = event.getObject().getGenericAttributes().get("logs", Integer.class);
            if (left == null) {
